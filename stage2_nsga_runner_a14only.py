...
RESULTS_DIR = r"C:\Users\Dell\Desktop\GRT\Codes\Stage2_NSAGII\Results\a14_n30"

ALT_ID = "A14"
BASE_ARCH = {"PV16_kW": 20.0, "PV17_kW": 6.0, "WT16_kW": 15.0, "BESS_P_kW": 30.0, "BESS_E_kWh": 120.0, "MT_kW": 30.0, "FC_kW": 10.0}
...
def plot_pareto(path_png: str, path_pdf: str, F: np.ndarray, knee_point_xy: tuple, eps_point_xy: tuple, title: str):
    ...
    ax.scatter(F[:, 0], F[:, 1], s=18,color='steelblue', label="Pareto solutions")
    ax.scatter([knee_point_xy[0]], [knee_point_xy[1]], marker="^", s=55,color='red',zorder=5,label="Knee point")
    ax.scatter([eps_point_xy[0]], [eps_point_xy[1]], marker="x", s=55,color='green',zorder=5,label="$\\epsilon$-constraint")
...
def plot_validation_distributions(path_png: str, path_pdf: str, df_per: pd.DataFrame, title: str):
    
    _set_ieee_plot_style()
    plt.rcParams.update({"axes.titlesize": 8})
  
    df_plot = df_per.copy()
    if "Export_kWh" in df_plot.columns:
        exp = pd.to_numeric(df_plot["Export_kWh"], errors="coerce").fillna(0.0).values.astype(float)
        exp[np.abs(exp) < 1e-6] = 0.0
        df_plot["Export_kWh"] = exp

    keys = [
        ("Import_kWh", "Import (kWh)"),
        ("TotalCost_$yr", "Cost ($/yr)"),
        ("Vmin_pu", "Vmin (p.u.)"),
        ("Vmax_pu", "Vmax (p.u.)"),
        ("ViolBH", "ViolBH"),
    ...
    for i, (col, lab) in enumerate(keys):
        ...
        x = pd.to_numeric(df_plot[col], errors="coerce").dropna().values.astype(float)

       
        ax_i.boxplot(x, vert=True, showfliers=False)
        ax_i.set_title(lab, pad=6)
        ax_i.grid(True, alpha=0.3)

        
        if col == "Vmin_pu":
            ax_i.axhline(Vmin_lim, linestyle="--", linewidth=1.0)
        if col == "Vmax_pu":
            ax_i.axhline(Vmax_lim, linestyle="--", linewidth=1.0)

        
        if col == "Export_kWh" and x.size > 0 and np.allclose(x, 0.0):
            ax_i.set_ylim(-0.1, 0.1)
...
def run_a14_only(sim: Simulator):
    ...
    for name, idx in sel:
        ...
        obj_v, met_v, df_per = sim.evaluate_architecture(
            x=X[idx, :],
            base_arch=BASE_ARCH,
            scenario_cfg=scen_val,
            cost_cfg=cost_cfg,
            arch_id=f"{ALT_ID}_{name}",
            n_workers=0,
            export_tol_kwh=EXPORT_TOL_KWH,
            fc_pmin_frac=FC_PMIN_FRAC
        )
        ...
        met_v.update(ok)
        val_results.append((name, met_v, df_per))

        val_path = os.path.join(RESULTS_DIR, f"NSGA2_{ALT_ID}_ROBUST_VALIDATION_{timestamp()}_{name}.xlsx")
        ...
        dist_png = os.path.join(RESULTS_DIR, f"ValidationDist_{ALT_ID}_{name}_{timestamp()}.png")
        dist_pdf = os.path.join(RESULTS_DIR, f"ValidationDist_{ALT_ID}_{name}_{timestamp()}.pdf")
        plot_validation_distributions(
            dist_png, dist_pdf, df_per,
            title=f"Scenario spread (validation) — {ALT_ID} {name} (N={N_SCEN_VAL})"
        )
        print(f"✅ Saved validation distribution plot: {dist_png} and {dist_pdf}")

    pass_count = int(sum(int(v[1]["val_robust_ok_all"]) for v in val_results))
    print("\n--- Robust validation summary ---")
    print(f"Robust pass: {pass_count} / {len(val_results)}")


    pareto_path = os.path.join(RESULTS_DIR, f"NSGA2_SOFT_{ALT_ID}_Pareto_{timestamp()}.xlsx")
    save_pareto_excel(pareto_path, X, F_raw, mets, ALT_ID, k_idx, e_idx)
    print(f"\n✅ Finished {ALT_ID} | Pareto saved: {pareto_path}")

    
    png = os.path.join(RESULTS_DIR, f"Pareto_{ALT_ID}_{timestamp()}.png")
    pdf = os.path.join(RESULTS_DIR, f"Pareto_{ALT_ID}_{timestamp()}.pdf")
    plot_pareto(
        png, pdf, F_raw,
