...
CASE_TAG        = "Stage1_R1Slack_AllAlternatives_FINAL"
...
def read_lines(path):
    ...
    if SLACK_BUS not in set(raw["from_bus"]).union(set(raw["to_bus"])):
        print(f"WARNING: Slack {SLACK_BUS} not found in any line endpoints after parsing.")
...
def build_tree_strict(lines, slack="R1"):
    ...
    if missing:
        raise ValueError(
            f"Network is NOT fully connected from {slack}. Unreachable buses: {missing}\n"
            f"Fix your line sheet (from_bus,to_bus) so R1->R2->... is connected."
        )
    ...
    for _ in range(max_iter):
        ...
        for b in buses:
            if b == slack:
                Iinj[b] = 0j
                continue
            S = Sbus_pu.get(b, 0j)  # + means demand
...
def evaluate_one_alternative(
    alt,
    Pmat_kW, Qmat_kVAr,
    pv_per1kw, wt_per1kw,
    buses, parent, children, z, post,
    export_timeseries=False,
    out_dir=None,
    case_tag="Stage1",
):
    alt_id = alt["Alt_ID"]
    ...
    for t in range(360):
        ...
        if export_timeseries:
            rows.append({
                "t": t, "Day": (t//24)+1, "Hour": hod,
                "Pload_total_kW": Pload_total,
                "PV16_used_kW": Ppv16, "PV17_used_kW": Ppv17, "WT16_used_kW": Pwt16,
                "PV_curt_kW": (c_pv16 + c_pv17), "WT_curt_kW": c_wt16,
                "Pbess4_kW": Pbess4, "SoC": soc,
                "Pmt15_kW": Pmt15, "Pfc18_kW": Pfc18,
                "Pgrid_import_kW": Pgrid_import, "Pgrid_export_kW": max(Pgrid_export, 0.0),
                "Ploss_kW": Ploss_kW, "Vmin_pu": Vmin, "Vmax_pu": Vmax
            })
    ...
    ts_df = pd.DataFrame(rows) if export_timeseries else None
    if export_timeseries and out_dir is not None:
        ts_path = safe_to_excel(ts_df, os.path.join(out_dir, f"{case_tag}_{alt_id}_timeseries.xlsx"))
        print("Saved:", ts_path)
...
def run_optionB(
    ...
    for alt in alts:
        alt_id = alt["Alt_ID"]
        print(f"\n=== Option B: {alt_id}  ({cfg.N} scenarios) ===")
        ...
        for s in range(cfg.N):
            ...
            m, _ = evaluate_one_alternative(
                alt=alt,
                Pmat_kW=P_s, Qmat_kVAr=Q_s,
                pv_per1kw=pv_s, wt_per1kw=wt_s,
                buses=buses, parent=parent, children=children, z=z, post=post,
                export_timeseries=False
            )
            per_s_metrics.append(m)
            scenario_rows.append({"Alt_ID": alt_id, "Scenario": s, **m})

        robust = aggregate_metrics_over_scenarios(per_s_metrics, metric_specs)
        robust_rows.append({"Alt_ID": alt_id, **robust})

    robust_df = pd.DataFrame(robust_rows)
    scen_df   = pd.DataFrame(scenario_rows)

    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    out_path = os.path.join(OUT_DIR, f"{CASE_TAG}_OptionB_{cfg.N}scen_MeanWorst_{stamp}.xlsx")

    with pd.ExcelWriter(out_path, engine="openpyxl") as w:
        robust_df.to_excel(w, sheet_name="DecisionMatrix_MeanWorst", index=False)
        scen_df.to_excel(w, sheet_name="ScenarioMetrics_All", index=False)
        pd.DataFrame(load_mult).to_excel(w, sheet_name="LoadMultipliers", index=False)
        pd.DataFrame(pv_mult).to_excel(w, sheet_name="PVMulitpliers", index=False)
        pd.DataFrame(wt_mult).to_excel(w, sheet_name="WTMultipliers", index=False)

        if runinfo_dict is not None:
            pd.DataFrame([runinfo_dict]).to_excel(w, sheet_name="RunInfo", index=False)

    print("\n✅ Option B DONE.")
    print("✅ Saved robust decision matrix:", out_path)
    return out_path
...
def main():
    ...
    wind_path = safe_to_excel(wind_df, os.path.join(OUT_DIR, "Wind_15day_hourly_synthetic.xlsx"))
    print("Saved wind profile:", wind_path)

   
    alts = read_alternatives_exact(PATH_ALT_XLSX, SHEET_ALT)
    print(f"Loaded {len(alts)} alternatives from '{SHEET_ALT}'")

  
    if RUN_MODE.upper() == "DETERMINISTIC":
        summary_rows = []

        for alt in alts:
            alt_id = alt["Alt_ID"]
            print(f"\n=== Running {alt_id} (NO_EXPORT={ENFORCE_NO_EXPORT}) ===")

            metrics, _ = evaluate_one_alternative(
                alt=alt,
                Pmat_kW=Pmat_kW, Qmat_kVAr=Qmat_kVAr,
                pv_per1kw=pv_per_kw, wt_per1kw=wind_cf,
                buses=buses, parent=parent, children=children, z=z, post=post,
            ...
            summary_rows.append({
                "Alt_ID": alt_id,
                "Total_Loss_kWh": metrics["Loss_kWh"],
                "Min_V_pu": metrics["Vmin_pu"],
                "Max_V_pu": metrics["Vmax_pu"],
                "Voltage_Violation_BusHours": metrics["ViolBH"],
                "Grid_Import_kWh": metrics["Import_kWh"],
                "Grid_Export_kWh": metrics["Export_kWh"],
                "Renewable_Utilization_pct": metrics["RenUtil_pct"],
                "Curtailment_kWh_total": metrics["Curt_kWh"],
                "BESS_Cycles_est": metrics["BESS_cycles"],
            })

        summary = pd.DataFrame(summary_rows)
        summary_file = os.path.join(OUT_DIR, f"{CASE_TAG}_DecisionMatrix_Stage1.xlsx")
        saved = safe_to_excel(summary, summary_file)
        print("\n✅ DONE (DETERMINISTIC).")
        print("✅ Decision matrix saved:", saved)
        print("✅ Results folder:", OUT_DIR)
        return

   

if RUN_MODE.upper() == "OPTION_B":
    cfg = ScenarioConfig(N=50, seed=7)

    t1_main = time.time()
    runtime_s = t1_main - t0_main

    out_path = run_optionB(
        alts=alts,
        P_base=Pmat_kW,
        Q_base=Qmat_kVAr,
        pv_base_per1kw=pv_per_kw,
        wt_base_per1kw=wt_per_kw,
        buses=buses, parent=parent, children=children, z=z, post=post,
        cfg=cfg,
        runinfo_dict={
            "CaseTag": CASE_TAG,
            ...
            "Vmax_lim": Vmax_lim,
            "CPU_or_System": "",
            "Notes": "Baseline Stage-1 Option B runtime"
        }
    )

    print("Saved:", out_path)
    print(f"\nStage-1 baseline runtime = {runtime_s:.1f} s = {(runtime_s/60):.2f} min")
        return

    raise ValueError("RUN_MODE must be either 'DETERMINISTIC' or 'OPTION_B'.")
