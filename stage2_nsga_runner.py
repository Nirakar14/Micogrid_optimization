        ...
        def _evaluate(self, X, out, *args, **kwargs):
            ...
            for i in range(X.shape[0]):
                obj, met, _ = sim.evaluate_architecture(
                    ...
                    n_workers=0,  # N=10 => serial is fastest
...
def plot_pareto(path_png: str, path_pdf: str, F: np.ndarray, knee_point_xy: tuple, eps_point_xy: tuple, title: str):
    ...
    ax.scatter(F[:, 0], F[:, 1], s=18,color='steelblue',
...
def run_alt(sim: Simulator, alt_id: str, base_arch: dict):
    ...
    pass_count = int(sum(int(v["val_robust_ok_all"]) for v in val_results))
    ...
    plot_pareto(
        ...
        title=f"Pareto front — {alt_id}"
    )
    print(f"✅ Saved plot: {png} and {pdf}")
